...
class CIFAR10_Module(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        dataset = CIFAR10(root=self.hparams.data_dir, train=True, transform=transform_train, download=self.download_cifar)
    ...
    def features_dataloader(self, train=True):
        transform_train = transforms.Compose([transforms.ToTensor(),
                                              transforms.Normalize(self.mean, self.std)])

        dataset = CIFAR10(root=self.hparams.data_dir, train=train, transform=transform_train,
